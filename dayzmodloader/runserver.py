...
from PyQt5.QtWidgets import QApplication, QWidget, QFileDialog, QMessageBox, QInputDialog, QTableWidget
...
class ModLoaderApp(QWidget):
    def __init__(self):
        ...
        self.setWindowTitle("DayZ Mod Loader")
        # Constants
        self.MODS_JSON_PATH = "mods.json"
        self.PATHS_JSON_PATH = "paths.json"
        ...
        # Load server and workshop paths from JSON file
    ...
    def init_ui(self):
        ...
        vbox.addWidget(self.mods_label)
 ...
 # defs
    ...
    def add_dzconfig(self):
        mod_list_name = self.get_selected_mod_list_name()
        if not mod_list_name:
            QMessageBox.critical(self, "Error", "Please select a mod list.")
            return

        dzconfig_path, _ = QFileDialog.getOpenFileName(self, "Select config file", "", "cfg (*.cfg)")
        if dzconfig_path:
            store_dz_config(self.MODS_JSON_PATH, mod_list_name, dzconfig_path)
            self.mods[mod_list_name]["dz_config"] = dzconfig_path
            save_mods(self.MODS_JSON_PATH, self.mods)
            self.update_mod_and_config_tables()
        ...
        if file_dialog.exec_():
            ...
            for mod_path in selected_mod_paths:
                ...
                if not self.symlink_exists_in_other_mod_lists(mod_list_name, mod_symlink_path):
                    if not is_symlink:
                        try:
                            os.symlink(mod_path, mod_symlink_path)
                        except OSError as e:
    ...
    def update_mod_and_config_tables(self):
        ...
        if mod_list_name:
            ...
            for row, mod_path in enumerate(mod_list_mods):
                ...
                # Check if the mod path is a symbolic link
    ...
    def show_command_line(self, mod_list_name, server_flags):
        mod_list_name = self.get_selected_mod_list_name()
        if not mod_list_name:
            QMessageBox.critical(None, "Error", "Please select a mod list.")
            return
        ...
        # Combine all the mod names into a single string
        mod_names = ";".join(os.path.basename(mod_path) for mod_path in mod_list_mods)
        # Get server options
        server_options = self.mods.get(mod_list_name, {}).get("server_options", {})
        ...
        cmd = (
            f'-config=serverDZ.cfg -server -port=2302 -profiles="{folder_name}" '
    ...
    def run_server_command(self, cmd):
        ...
        server_exe_path = os.path.join(self.workshop_path, "..", "..", "DayZ", "DayZDiag_x64.exe")
    ...
    def run_client_command(self):
        ...
        # Construct the command line
        cmd = (f'DayZDiag_x64.exe "-mod={mod_names}"')
        ...
        msg_box = QtWidgets.QMessageBox()
        msg_box.setWindowTitle("Command Line")
        msg_box.setText("The command line to start the client is:")
    ...
    def launch_client(self, cmd):
        client_exe_path = os.path.join(self.workshop_path, "..", "..", "DayZ", "DayZ_BE.exe")
        print(f"Client executable path: {client_exe_path}")
